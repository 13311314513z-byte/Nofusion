...
export function TruthFiles({ bookId, nav, theme, t }: { bookId: string; nav: Nav; theme: Theme; t: TFunction }) {
  ...
  const { data, refetch: refetchFiles } = useApi<{ files: ReadonlyArray<TruthFile> }>(`/books/${bookId}/truth`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">{t("truth.manualEditHint")}</p>
      </div>
          ...
          {data?.files.map((f) => (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                {f.legacy && (
                  <span className="shrink-0 rounded border border-amber-500/40 px-1.5 py-0.5 text-[10px] text-amber-600 dark:text-amber-300">
                    {t("truth.readOnly")}
                  </span>
                )}
              </div>
