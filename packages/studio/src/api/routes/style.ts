...
import {
  loadSecrets, setServiceApiKey, saveSecrets,
  isDocumentFileType, buildFoundationSourceBundle,
  persistFoundationSourceBundle, type StyleFingerprint,
...
// --- Style Analyze ---
  ...
  app.get("/api/v1/books/:id/chapter-goals", async (c) => {
    ...
    try {
      const ctx.state = new StateManager(root);
  ...
  app.get("/api/v1/books/:id/interview", async (c) => {
    ...
    try {
      ...
      try {
        const { readFile } = await import("node:fs/promises");
        const { join } = await import("node:path");
        const hooksPath = join(bookDir, "story", "ctx.state", "hooks.json");
