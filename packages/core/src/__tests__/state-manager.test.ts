...
describe("StateManager", () => {
  ...
  describe("ensureControlDocuments", () => {
    ...
    it("repairs poisoned current state chapter when it runs ahead of durable progress", async () => {
      const bookId = "runtime-state-current-ahead-book";
      const bookDir = manager.bookDir(bookId);
      const storyDir = join(bookDir, "story");
      const stateDir = join(storyDir, "state");
      const chaptersDir = join(bookDir, "chapters");
      ...
      await mkdir(chaptersDir, { recursive: true });
      await Promise.all([
        writeFile(join(chaptersDir, "0001_opening.md"), "# Opening\n", "utf-8"),
        ...
        writeFile(
          ...
          [
            ...
            "| --- | --- |",
            "| Current Chapter | 4 |",
            "| Current Goal | Future poisoned goal |",
            "",
          ].join("\n"),
          "utf-8",
        ),
      ...
      await manager.ensureRuntimeState(bookId);

      const manifest = JSON.parse(
        await readFile(join(stateDir, "manifest.json"), "utf-8"),
      ) as { lastAppliedChapter: number; migrationWarnings: string[] };
      const currentState = JSON.parse(
        await readFile(join(stateDir, "current_state.json"), "utf-8"),
      ) as { chapter: number; facts: Array<{ object: string }> };

      expect(manifest.lastAppliedChapter).toBe(1);
      expect(manifest.migrationWarnings.join("\n")).toContain("current_state chapter normalized from 4 to 1");
      expect(currentState.chapter).toBe(1);
      expect(currentState.facts.map((fact) => fact.object)).toEqual(["Opening room"]);
    });
