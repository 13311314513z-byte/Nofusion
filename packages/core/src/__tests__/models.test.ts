...
describe("ChapterMetaSchema", () => {
  ...
  it("accepts structured chapter metadata defaults and fields", () => {
    ...
    expect(result.tags).toEqual(["battle", "turn"]);
    expect(result.povCharacter).toBe("Ari");
    expect(result.location).toBe("North Gate");
    expect(result.moodScore).toBe(-3);
    expect(result.chapterType).toBe("conflict");
    expect(result.timeOfDay).toBe("night");
    expect(result.revisionCount).toBe(2);
    expect(result.wordCountTarget).toBe(3200);
  });
