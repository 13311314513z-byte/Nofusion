...
import { appendAuditHistory } from "../utils/audit-history.js";
...
export async function runChapterReviewCycle(params: {
  ...
  const assess = async (
    ...
    // Persist audit history (fire-and-forget; failures are non-blocking)
    const round = auditRound;
    auditRound++;
    void appendAuditHistory(params.bookDir, params.chapterNumber, auditResult, round).catch(() => {
      // Silently ignore write failures — audit history is best-effort telemetry
    });
