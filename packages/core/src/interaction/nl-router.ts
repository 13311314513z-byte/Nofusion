...
export function routeNaturalLanguageIntent(
  ...
  // === Natural language continue/write-next (prefix match, not exact) ===
  if (/^(?:继续|继续写|写下一章|写下一|再写|再写一章|往下写|接下来写|continue|write next|write the next|write another)/i.test(trimmed)) {
    return {
      intent: "write_next",
      ...(bookId ? { bookId } : {}),
    };
  ...
  // Natural-language focus update — requires explicit command prefix to avoid
  // mis-triggering on casual mentions of "主线" (main storyline) in chat.
  const focusNaturalMatch = trimmed.match(/^(?:聚焦|关注|切换到|调整)\s*(?:到|在|于)?\s*(.+)$/i) ??
    trimmed.match(/^(?:focus on|set focus to|switch focus to)\s+(.+)$/i) ??
    trimmed.match(/^[把将]\s*focus\s*(?:拉回|转到|切换到|调整|改成)\s*(.+)$/i);
  if (focusNaturalMatch) {
    return {
      intent: "update_focus",
      ...(bookId ? { bookId } : {}),
      instruction: trimmed,
  ...
  // Help/greeting queries without an active book → show welcome message.
  if (!bookId && /^(?:help|帮助|能做什么|你会什么|功能|命令|commands|what can you do|how (?:to|do I)|怎么用)/i.test(trimmed)) {
    return {
      intent: "chat",
      instruction: trimmed,
    };
  ...
  // Fallback: no active book → assume book creation (the chat handler
  // can also detect creation intent from the AI response).
  if (!bookId) {
    return {
      intent: "develop_book",
      instruction: trimmed,
  ...
  // With a creation draft pending, route to develop_book.
