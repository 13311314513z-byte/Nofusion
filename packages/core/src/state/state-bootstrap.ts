...
export async function bootstrapStructuredStateFromMarkdown(params: {
  ...
  const repairedCurrentState = repairCurrentStateProgress(currentState, derivedProgress, warnings);
  if (repairedCurrentState.changed) {
    currentState = repairedCurrentState.state;
...
function repairCurrentStateProgress(
  currentState: CurrentStateState,
  durableProgress: number,
  warnings: string[],
): { readonly state: CurrentStateState; readonly changed: boolean } {
  if (currentState.chapter <= durableProgress) {
    return { state: currentState, changed: false };
  }

  appendWarning(
    warnings,
    `current_state chapter normalized from ${currentState.chapter} to ${durableProgress}`,
  );

  const facts = currentState.facts.filter((fact) =>
    fact.validFromChapter <= durableProgress && fact.sourceChapter <= durableProgress);
  if (facts.length !== currentState.facts.length) {
    appendWarning(
      warnings,
      `current_state future facts pruned from ${currentState.facts.length} to ${facts.length}`,
    );
  }
